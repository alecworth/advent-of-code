let arr = ['dssmtmrkonedbbhdhjbf9hq',
...
// split arr by each space in string
let splitArr = (arr.map((element) => element.split('')))

// create new array to insert numbers from the strings
let newArr = []

//iterate through each array within splitArr
//check if element is a number, if so push it
for (let i = 0; i < splitArr.length; i++) {
    newArr.push(splitArr[i].filter((element) => !isNaN(element)))
}

//take first and last instances of numbers converted from strings into numbers
newArr = newArr.map((element) => Number(element[0] + element [element.length - 1]))
